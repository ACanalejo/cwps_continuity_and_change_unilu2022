...
- To follow these updates, we will first some basic notions of the **spatial theory of voting and party competition**
...
- We will use this basic setup for simplicity, although **Europe** is mostly characterize by **multiparty systems**

## Introduction to the spatial model I: the unidimensional model

- So first of all, by do we mean by a **dimension**?
...
- The **main assumption of the Downsian model** is that parties aim to **maximize votes** to win the election

- The second assumption is that each voter have a fixed policy preference over the continuum, which gives place to a **preference distribution**

## Introduction to the spatial model I: the unidimensional model

- Each party depart from a different side of the spectrum (as representing **each side of a cleavage**)

- Then, each political party take the **policy stance** (i.e., position over the continuum) that maximizes its chances of winning the election by **approximating the preference of the median voter** (i.e., the preference that leaves an equal number of voters at each side of the distribution)

- Let's imagine in our case that we have two parties, each of them representing each side of the left-right ideological dimension that want to maximize their chances of winning the next election by moderating their positions

  1. The first party is a **centre-left party**, that takes a **4**

  2. The second party is a **centre-right party**, that takes a **5**
...
- Let's play with this set-up a little!
  ...
  9. What happens if the position of the parties is **3 and 7** respectively and they have a **programmatic constraints/reputation** (i.e., they cannot credibly shift without a cost)?

## Introduction to the spatial model I: the unidimensional model

- In sum, **the Downsian model**, as well as models in general, is **a tool that help us understand the reality**, by providing means of mathematically and graphically summarising (theoretical) expectations

- **Transforming cleavages into dimensions of competition** help us to **analyse the interaction between parties (i.e., party systems)** as well as **parties and voters (i.e., electoral competition)**, under a given set of conditions

# Introduction to the spatial model II: the bidimensional model

## Introduction to the spatial model II: the bidimensional model

- So far, however, we have only contemplated situations in which party compete over a single dimensions

- However, we know that **cleavages can overlap or cross-cut each other**

- If they don't completely overlap, we can interpret the secondary **cross-cutting cleavage as a second dimension**

- Let's say for example that parties compete in **Catalonia (Spain)** where there are two important dimensions

  1. The **left-right economic dimension** is rooted in the **class cleavage**
  
  2. The **integration-demarcation dimension** is rooted in the **centre-periphery cleavage**
...
This lead to a series of implications, most importantly:
  ...
  2. The **larger** becomes the **space opened for new parties**, the **party system** will tened to be **more fragmented**
...
- ***Basic features*** of the new cleavage:
  ...
  1. The cleavage is rooted on  (1) the expansion of **educational rights**, (2) the transition **from an industrial to a post-industrial society** and (3) the processes of **globalization**
  ...
  4. The parties that better represent each of the poles of the conflict are the **new left parties** for the winners and the **PRR** parties for the losers
...
- There are important regional differences:
  ...
  1. In ***NWE***, the second and first dimensions are **more orthogonal** and **new parties** have been the main drivers of shift to the second dimension
  
  2. In ***SE***, the second and first dimensions are **less orthogonal** and **traditional** parties have more frequently drove the shift to the second dimension
...
- **Final activity!** Let look at two paradigmatic cases visualized with real data from 15 years ago: **what can we say about them? What do they tell recent events?**
